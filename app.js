...
var contadoresFotos={VP:{},EI:{},EL:{}};
var contadorFotosVP=contadoresFotos.VP,contadorFotosEV=contadoresFotos.EI,contadorFotosEL=contadoresFotos.EL;
...
// --- Fetch autenticado con manejo de sesión expirada ---
...
function generarTokenSeguro(){var a=new Uint8Array(32);crypto.getRandomValues(a);return Array.from(a,function(b){return b.toString(16).padStart(2,'0');}).join('');}
// Hash simple para contraseñas locales (SHA-256, no reversible)
function hashPassword(password){
  if(typeof crypto!=='undefined'&&crypto.subtle){
    return crypto.subtle.digest('SHA-256',new TextEncoder().encode(password)).then(function(buf){return Array.from(new Uint8Array(buf),function(b){return b.toString(16).padStart(2,'0');}).join('');});
  }
  // Fallback: simple hash para navegadores viejos
  var hash=0;for(var i=0;i<password.length;i++){hash=((hash<<5)-hash)+password.charCodeAt(i);hash|=0;}
  return Promise.resolve('fallback_'+Math.abs(hash).toString(16));
}
...
function getUsuariosLocal(){var d=localStorage.getItem('rapca_usuarios_local');return safeJSONParse(d,[]);}
function guardarUsuariosLocal(lista){try{localStorage.setItem('rapca_usuarios_local',JSON.stringify(lista));}catch(e){console.error('Error guardando usuarios:',e);showToast('Error guardando datos locales','error');}}
function initUsuariosLocal(){
  var users=getUsuariosLocal();
  // Migrar contraseñas plaintext a hash
  var needsMigration=false;
  users.forEach(function(u){
    if(u.password&&!u.passwordHash){needsMigration=true;}
  });
  if(needsMigration){
    var pending=users.filter(function(u){return u.password&&!u.passwordHash;});
    var migrateNext=function(idx){
      if(idx>=pending.length)return;
      hashPassword(pending[idx].password).then(function(h){
        pending[idx].passwordHash=h;
        delete pending[idx].password;
  ...
  if(users.length===0){
    hashPassword('Gallito9431%').then(function(h){
      users.push({id:1,email:'rapcajaen@gmail.com',nombre:'Administrador',passwordHash:h,rol:'admin',activo:1});
      guardarUsuariosLocal(users);
    });
    return;
  }
  // Migrar admin antiguo
  ...
  if(oldAdmin){
    ...
    hashPassword('Gallito9431%').then(function(h){
      oldAdmin.passwordHash=h;delete oldAdmin.password;
      guardarUsuariosLocal(users);
    });
  }
...
function loginLocal(email,password){
  ...
  // Compatibilidad: soportar tanto passwordHash como password legacy
  var user=users.find(function(u){return u.email.toLowerCase()===emailNorm&&u.activo!==0;});
  if(!user)return{ok:false,error:'Email o contraseña incorrectos'};
  // Si aún tiene password sin hash (migración pendiente), comparar y migrar
  if(user.password&&!user.passwordHash){
    if(user.password===password.trim()||user.password===password){
      // Migrar a hash inmediatamente y eliminar plaintext
      return hashPassword(password.trim()).then(function(h){
        user.passwordHash=h;delete user.password;
        guardarUsuariosLocal(users);
        return{ok:true,token:generarTokenSeguro(),usuario:{email:user.email,nombre:user.nombre,rol:user.rol,id:user.id}};
      });
    }
    return Promise.resolve({ok:false,error:'Email o contraseña incorrectos'});
  }
  // Comparar con hash (async → devuelve promesa)
  return hashPassword(password.trim()).then(function(h){
...
function crearUsuarioLocal(email,nombre,password,rol){
  var users=getUsuariosLocal();
  var emailNorm=email.trim().toLowerCase();
  if(users.find(function(u){return u.email.toLowerCase()===emailNorm;}))return Promise.resolve({ok:false,error:'Email ya existe'});
  return hashPassword(password.trim()).then(function(h){
    users.push({id:Date.now(),email:emailNorm,nombre:nombre,passwordHash:h,rol:rol||'operador',activo:1});
    guardarUsuariosLocal(users);
    return{ok:true};
  });
}
...
function subirFotoNube(codigo,dataUrl,unidad,tipo){
  if(!isOnline)return;
  var token=sesionActual?sesionActual.token:'';
  var payload=JSON.stringify({codigo:codigo,imagen:dataUrl,unidad:unidad,tipo:tipo,token:token});
...
function subirConReintentos(codigo,dataUrl,unidad,tipo,maxI,cb){
  ...
  function intentar(){
    ...
    var tkn=sesionActual?sesionActual.token:'';fetchAuth(UPLOAD_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({codigo:codigo,imagen:dataUrl,unidad:unidad,tipo:tipo,token:tkn})}).then(function(r){if(!r.ok)return r.text().then(function(t){var msg=extraerErrorSubida(t);throw new Error('HTTP '+r.status+': '+msg);});return r.json();}).then(function(d){if(d.ok){eliminarSubidaPendiente(codigo);console.log('Subida OK: '+codigo+(d.url?' → '+d.url:''));cb(true);}else throw new Error(d.error||'Error servidor');}).catch(function(e){console.error('Intento '+i+'/'+maxI+' '+codigo+':',e.message);if(i<maxI&&isOnline)setTimeout(intentar,Math.pow(2,i)*1000);else{showToast('❌ '+codigo+': '+e.message,'error');cb(false);}});
  }
  intentar();
}
...
function notificarFalloAdmin(codigos){
  ...
  var ntkn=sesionActual?sesionActual.token:'';fetch('notificar.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({codigos:codigos,error:'Fallo tras 3 reintentos por foto',dispositivo:navigator.userAgent,token:ntkn})}).catch(function(){});
}
...
function actualizarMarcadoresMapa(){
  ...
  if(filtroTipo!=='infra'){
    ...
    rs.forEach(function(r){
      ...
      mk.bindPopup('<b>'+escapeHTML(r.tipo)+' '+escapeHTML(r.unidad)+'</b><br>'+escapeHTML(r.fecha)+(r.operador_nombre?'<br>'+escapeHTML(r.operador_nombre):''));
  ...
  if(filtroTipo!=='infra'){
    ...
    allRs.forEach(function(r){
      ...
      d.fotosComp.forEach(function(fc){
        ...
        mk.bindPopup('<b>📷 '+escapeHTML(fc.waypoint)+' — '+escapeHTML(r.unidad)+'</b><br>'+escapeHTML(r.fecha)+'<br>Fotos: '+escapeHTML(codigos.join(', ')));
        clusterGroup.addLayer(mk);
      });
    });
  }
  ...
  if(!filtroTipo||filtroTipo==='infra'){
    ...
    infras.forEach(function(inf){
      ...
      mk.bindPopup('<b>'+escapeHTML(idLabel)+'</b><br>'+escapeHTML(inf.nombre||'')+'<br>'+escapeHTML(inf.municipio||'')+'<br><small>VP: '+vpCount+' | EL: '+elCount+' | EI: '+eiCount+' | Fotos: '+fotoCount+'</small>');
      clusterGroup.addLayer(mk);
    });
  }
}
  ...
  Object.keys(capasKML).forEach(function(nombre){
    ...
    if(totalSub>0){
      ...
      subcapas.forEach(function(sc,idx){
        ...
        html+='<td class="col-nombre" title="'+escapeHTML(sc.desc||scNombre)+'">'+escapeHTML(scNombre)+'</td>';
...
function mostrarSelectorCampoEtiqueta(nombre,campos){
  ...
  campos.forEach(function(campo){
    ...
    h+='<button class="label-field-btn" data-field="'+escapeHTML(campo)+'" style="display:block;width:100%;text-align:left;padding:10px 14px;margin-bottom:6px;border:1px solid #ddd;border-radius:6px;background:#f9f9f9;cursor:pointer;font-size:.9rem"><strong>'+escapeHTML(campo)+'</strong>'+(ejemplo?'<br><span style="color:#888;font-size:.8rem">ej: '+escapeHTML(ejemplo)+'</span>':'')+'</button>';
  });
  list.innerHTML=h;
...
function buscarEnMapa(query,resultsId){
  ...
  resultados.forEach(function(r,idx){
    ...
    html+='<div class="sr-info"><div class="sr-title">'+escapeHTML(r.label)+'</div><div class="sr-sub">'+escapeHTML(r.sub)+'</div></div>';
    html+='</div>';
  });
...
function aceptarFoto(){
  ...
  canvas.toBlob(function(b){var u=URL.createObjectURL(b);var l=document.createElement('a');l.href=u;l.download=codigo+'.jpg';l.click();setTimeout(function(){URL.revokeObjectURL(u);},5000);},'image/jpeg',0.95);
...
function procesarLoginLocal(email,pass,msgError){
  var btn=document.getElementById('btnLogin');
  var result=loginLocal(email,pass);
  // loginLocal puede devolver un objeto o una Promise
  Promise.resolve(result).then(function(r){
    btn.textContent='Entrar';btn.disabled=false;
    if(r.ok){
      sesionActual={token:r.token,email:r.usuario.email,nombre:r.usuario.nombre,rol:r.usuario.rol,id:r.usuario.id};
      localStorage.setItem('rapca_sesion',JSON.stringify(sesionActual));
      ocultarLoginMostrarApp();
    }else{mostrarErrorLogin(msgError||r.error||'Email o contraseña incorrectos');}
  });
}
function iniciarSesion(){
  ...
  if(isOnline){
    ...
    .then(function(data){
      btn.textContent='Entrar';btn.disabled=false;
      if(data.ok){
        // Guardar hash local para futuros logins offline
        hashPassword(pass).then(function(h){
          var users=getUsuariosLocal();
          var existing=users.find(function(u){return u.email.toLowerCase()===email;});
          if(existing){existing.id=data.usuario.id;existing.nombre=data.usuario.nombre;existing.passwordHash=h;delete existing.password;existing.rol=data.usuario.rol;existing.activo=1;}
          else{users.push({id:data.usuario.id,email:email,nombre:data.usuario.nombre,passwordHash:h,rol:data.usuario.rol,activo:1});}
          guardarUsuariosLocal(users);
        });
    ...
    .catch(function(e){
      procesarLoginLocal(email,pass,'Sin conexión al servidor y cuenta no encontrada en este dispositivo');
    });
  }else{
    procesarLoginLocal(email,pass,'Sin conexión. Cuenta no encontrada en este dispositivo');
  }
}
...
function cerrarSesion(){
  ...
  ['VP','EI','EL'].forEach(function(t){var c=contadoresFotos[t];Object.keys(c).forEach(function(k){delete c[k];});});
  // Limpiar estado de cámara
  if(cameraStream){try{cameraStream.getTracks().forEach(function(t){t.stop();});}catch(e){}cameraStream=null;}
  anotaciones=[];modoAnotacion=false;
  // Limpiar cache de fotos en memoria
  fotosCacheMemoria={};
  // Cerrar modales abiertos
  ['cameraModal','previewModal'].forEach(function(id){var el=document.getElementById(id);if(el)el.classList.remove('show');});
...
function crearUsuario(){
  ...
  if(isOnline){
    ...
    .then(function(data){
      if(data.ok){
        // Guardar hash local con ID del servidor
        hashPassword(pass).then(function(h){
          var users=getUsuariosLocal();
          if(!users.find(function(u){return u.email.toLowerCase()===email.toLowerCase();})){
            users.push({id:data.id,email:email.toLowerCase(),nombre:nombre,passwordHash:h,rol:rol||'operador',activo:1});
            guardarUsuariosLocal(users);
          }
        });
    ...
    .catch(function(){
      ...
      crearUsuarioLocal(email,nombre,pass,rol).then(function(localResult){
...
function cargarRegistrosServidor(){
  ...
  .then(function(data){
    ...
    data.registros.slice(0,100).forEach(function(r){
      h+='<div class="record-item"><span class="tipo '+escapeHTML(r.tipo).toLowerCase()+'">'+escapeHTML(r.tipo)+'</span> <strong>'+escapeHTML(r.zona)+'>'+escapeHTML(r.unidad)+'</strong>'+(r.transecto?' ('+escapeHTML(r.transecto)+')':'');
      h+='<div class="info">'+escapeHTML(r.fecha)+' | '+escapeHTML(r.usuario_nombre||r.usuario_email)+'</div></div>';
    });
    el.innerHTML=h;
  }).catch(function(){});
}
...
function sincronizarRegistroServidor(registro){
  ...
  fetchAuth(DATOS_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'guardar',token:sesionActual.token,registro:registro})})
...
function sincronizarInfraServidor(infra){
  if(!tieneTokenServidor()||!isOnline)return;
  fetchAuth(DATOS_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'guardar_infra',token:sesionActual.token,infra:infra})})
...
function guardarBorradores(){try{localStorage.setItem('rapca_borrador_vp',JSON.stringify(obtenerDatosVP()));localStorage.setItem('rapca_borrador_ev',JSON.stringify(obtenerDatosEV()));localStorage.setItem('rapca_borrador_el',JSON.stringify(obtenerDatosEL()));}catch(e){console.warn('Error guardando borradores:',e.message);}}
...
// --- Helpers compartidos VP/EL (mismos campos, distinto prefijo) ---
var CAMPOS_VPEL=['fecha','zona','unidad','past1','past2','past3','senal','veredas','cagarrutas','fotos','fc1','fc2','obs'];
var CAMPOS_FOTOS_VPEL=['fotos','fc1','fc2'];
function _obtenerDatosSimple(pre){var d={};CAMPOS_VPEL.forEach(function(k){d[k]=document.getElementById(pre+'-'+k).value;});return d;}
function _cargarDatosSimple(pre,d){CAMPOS_VPEL.forEach(function(k){if(d[k])document.getElementById(pre+'-'+k).value=d[k];});CAMPOS_FOTOS_VPEL.forEach(function(k){if(d[k]){var lid=pre+'-'+k+'-lista';if(k==='fotos')lid=pre+'-fotos-lista';actualizarListaFotos(lid,d[k]);}});}
function _limpiarFormSimple(pre,tipoContador,borradorKey){var t=new Date().toISOString().split('T')[0];document.getElementById(pre+'-fecha').value=t;document.getElementById(pre+'-zona').value='';document.getElementById(pre+'-unidad').value='';[pre+'-past1',pre+'-past2',pre+'-past3',pre+'-senal',pre+'-veredas',pre+'-cagarrutas',pre+'-fotos',pre+'-fc1',pre+'-fc2',pre+'-obs'].forEach(function(id){document.getElementById(id).value='';});[pre+'-fotos-lista',pre+'-fc1-lista',pre+'-fc2-lista'].forEach(function(id){document.getElementById(id).innerHTML='';});var ctr=contadoresFotos[tipoContador];Object.keys(ctr).forEach(function(k){delete ctr[k];});localStorage.setItem('rapca_contadores_'+tipoContador,JSON.stringify({}));localStorage.removeItem(borradorKey);}
function _guardarSimple(pre,tipo,tipoContador,limpiarFn){var z=document.getElementById(pre+'-zona').value.trim(),u=document.getElementById(pre+'-unidad').value.trim();if(!z||!u){showToast('Zona y Unidad obligatorios','error');return;}var d={pastoreo:[document.getElementById(pre+'-past1').value,document.getElementById(pre+'-past2').value,document.getElementById(pre+'-past3').value],observacionPastoreo:{senal:document.getElementById(pre+'-senal').value,veredas:document.getElementById(pre+'-veredas').value,cagarrutas:document.getElementById(pre+'-cagarrutas').value},fotos:document.getElementById(pre+'-fotos').value,fotosComp:[{numero:document.getElementById(pre+'-fc1').value,waypoint:'W1'},{numero:document.getElementById(pre+'-fc2').value,waypoint:'W2'}],observaciones:document.getElementById(pre+'-obs').value};var r={id:editandoId||Date.now(),tipo:tipo,fecha:document.getElementById(pre+'-fecha').value,zona:z,unidad:u,transecto:'',datos:d,enviado:false,lat:currentLat,lon:currentLon};if(sesionActual){r.operador_email=sesionActual.email;r.operador_nombre=sesionActual.nombre;}if(editandoId){actualizarRegistro(r);editandoId=null;}else guardarLocal(r);showToast(tipo+' guardado','success');limpiarFn();if(isOnline){enviarRegistro(r);sincronizarRegistroServidor(r);}}
function obtenerDatosVP(){return _obtenerDatosSimple('vp');}
function cargarDatosVP(d){_cargarDatosSimple('vp',d);}
...
function getGanaderos(){return safeJSONParse(localStorage.getItem('rapca_ganaderos'),[]);}
...
// --- Infraestructuras ---
function getInfras(){return safeJSONParse(localStorage.getItem('rapca_infraestructuras'),[]);}
...
function ejecutarBusqueda(){
  ...
  resultados.slice(0,30).forEach(function(r){
    ...
    h+='<div><div class="sr-title">'+escapeHTML(r.titulo)+'</div><div class="sr-sub">'+escapeHTML(r.sub)+'</div></div></div>';
  });
...
async function descargarZIPUnidad(unidad){
  ...
  zip.generateAsync({type:'blob'}).then(function(blob){
    var bUrl=URL.createObjectURL(blob);var a=document.createElement('a');a.href=bUrl;a.download='RAPCA_'+unidad+'_fotos.zip';a.click();setTimeout(function(){URL.revokeObjectURL(bUrl);},5000);
...
async function descargarZIPTodas(){
  ...
  zip.generateAsync({type:'blob'}).then(function(blob){
    var bUrl=URL.createObjectURL(blob);var a=document.createElement('a');a.href=bUrl;a.download='RAPCA_todas_fotos.zip';a.click();setTimeout(function(){URL.revokeObjectURL(bUrl);},5000);
...
function exportarKML(){
  ...
  var bUrl=URL.createObjectURL(blob);var a=document.createElement('a');a.href=bUrl;a.download='RAPCA_Campo.kml';a.click();setTimeout(function(){URL.revokeObjectURL(bUrl);},5000);
...
function renderTablaAtributos(){
  ...
  datos.forEach(function(fila){
    ...
    attrTableColumnas.forEach(function(col){
      var v=fila[col]||'';
      if(col==='Tipo')h+='<td class="attr-tipo-cell">'+(iconos[v]||escapeHTML(v))+'</td>';
      else if(col==='Lat'||col==='Lon')h+='<td class="attr-coord">'+escapeHTML(v)+'</td>';
      else h+='<td title="'+escapeHTML(String(v))+'">'+escapeHTML(v)+'</td>';
    });
    h+='</tr>';
  });
  tbody.innerHTML=h;
